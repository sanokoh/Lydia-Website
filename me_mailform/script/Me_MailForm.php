 ...
 *
 * @copyright Copyright (C) 2014-2019 MicroEngine Inc.
 * @version 1.1.5
 */
...
class Me_MailForm
{
    ...
    private function set_item_value()
    {
        ...
        foreach ($this->form_item as &$item) {
            if (strlen($item['get_param']) > 0 && isset($_GET[$item['get_param']])) {
        ...
        foreach ($this->form_item as $name => &$item) {
            ...
            if ($item['type'] === 'file') {
                if ($_FILES[$name]['error'] !== UPLOAD_ERR_OK) {
                    ...
                    // ファイル名・拡張子取得
                    $save_file_name = $_FILES[$name]['name'];
            ...
            if ($item['minlength'] && strlen($item['value']) > 0 && mb_strlen($item['value']) < $item['minlength']) {
            ...
            // 必須項目チェック
            if ($item['required'] && !$this->validate_has_value($item['value'])) {
            ...
            if ($item['required_if']) {
                ...
                if (isset($conditions[1]) ) {
                    ...
                    if ($matched && strlen($item['value']) === 0) {
            ...
            // 半角数字のみかどうかチェックする
            if ($item['numeric'] && strlen($item['value']) > 0 && !preg_match('/\A[0-9]*\z/u', $item['value'])) {
                $this->set_error_message($item, 'msg_numeric', array('{label}'=>$item['label']));
                $this->is_error = true;
                continue;
            }
            // 電話番号かどうかチェックする
            if ($item['phone'] && strlen($item['value']) > 0 && !preg_match('/\A\d{2,5}-\d{1,4}-\d{4}\z/u', $item['value'])) {
                $this->set_error_message($item, 'msg_phone', array('{label}'=>$item['label']));
                $this->is_error = true;
                continue;
            }
            // 郵便番号かどうかチェックする
            if ($item['postal'] && strlen($item['value']) > 0 && !preg_match('/\A\d{3}-\d{4}\z/u', $item['value'])) {
            ...
            if (strlen($item['regex']) > 0 && strlen($item['value']) > 0) {
            ...
            if (strlen($item['regex_not']) > 0 && strlen($item['value']) > 0) {
    ...
    private function output_item_value($html)
    {
        foreach ($this->form_item as $name => $item) {
            ...
            switch ($item['type']) {
                case 'textarea':
                    $item_elem->innertext = nl2br($this->html_escape($item['value']));
                    break;
                case 'select':
                ...
                case 'checkbox':
                    ...
                    break;
                case 'price':
                    $item_elem->innertext = nl2br($this->html_escape(number_format($item['value'])));
                    break;
                default:
    ...
    private function send_mail()
    {
        ...
        $this->form_item['_cc'] = array('type'=>'reserved','label'=>'Cc','value'=>$this->mail_config['mail']['cc']);
        $this->form_item['_bcc'] = array('type'=>'reserved','label'=>'Bcc','value'=>$this->mail_config['mail']['bcc']);
        ...
        if (!empty($this->config['upload_file']['attach_mail'])) {
            $tmpfile_list = array();
            foreach ($this->form_item as $name =>  $item) {
                if ($item['type'] === 'file' && !empty($item['value']) && isset($_SESSION['attach_files'][$name])) {
                    ...
                    $tmp_filename = $meta_data['uri'];
                    $attaches[] = array($tmp_filename , $item['value']);
                }
            }
        }
        if (!empty($attaches)) {
    ...
    private function save_file()
    {
        ...
        foreach ($this->form_item as $name =>  $item) {
            if ($item['type'] === 'file' && !empty($item['value']) && isset($_SESSION['attach_files'][$name])) {
                ...
                // ファイル名置換 {filename}
                $upload_file_path = str_replace('{filename}', $item['value'], $upload_file_path);
                // 拡張子置換 {ext}
