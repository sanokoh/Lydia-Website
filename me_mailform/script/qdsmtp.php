...
//	var $smtp_auth_kind	= array('CRAM-MD5','DIGEST-MD5','LOGIN','PLAIN');
	...
	function continueConnect( $bool = null ){
		...
		if( is_bool( $bool ) ){
			...
			return $this->errorGather('Connection Continue Mode specified error',__LINE__);
		}
	}
			...
			case 'SMTP_AUTH':
				...
				if( !$this->already_auth ){
					...
					if( 0 === preg_match('/[\s-]AUTH\s+([^\r\n]*)\r?\n/is', implode($this->smtpLFC,$this->smtp_log) , $matches)){
						return $this->errorGather('HOST:'.$this->smtp_param['HOST'].' does not supported SMTP AUTH Protocol',__LINE__);
					}
					...
					if( is_null( $decide ) ){
						return $this->errorGather('HOST:'.$this->smtp_param['HOST'].' does not supported MY Available SMTP AUTH Protocol '.implode(' or ',$this->smtp_auth_kind),__LINE__);
					}
...
*/
	...
	function sayHello(){
		$host = preg_replace('|^.*://|', '', $this->smtp_param['HOST']);
		$items = array(
			array( 'EHLO' , $host ),
			array( 'HELO' , $host ),
		);
		if( !$this->tryUntilSuccess( $items ) ){
			return $this->errorGather('HOST:'.$host.' say no HELLO',__LINE__);
		}
		return $this->errorGather();
	}
	function sendData(){
	...
	function tryUntilSuccess( $items ){
		...
		if( !$try ){
			return $this->errorGather( 'Try Error ->' . implode( ' ' , $err_mes ) ,__LINE__);
		}
		return $this->errorGather();
	}
	...
	function pop3(){
		...
		if( false === $fp ){
			return $this->errorGather('Can not connect POP3 Server' ,__LINE__);
		}
		$items = array(
