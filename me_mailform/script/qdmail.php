...
//****************************************************
	...
	function easy( $type , $to , $subject , $content , $other_header = array() , $attach = null ){
		...
		if( 'TEXT' == strtoupper( $type ) || 'HTML' == strtoupper( $type ) ){
			...
			$this->error[]='Illegal specify \'type\' in '.$type.' .'.__LINE__;
...
/*
 * Notice: Before use $this->optionNameLink(); by Constructor
*/
	//
	//---------------------------------------
	...
	function qd_convert_encoding( $word , $target_chrset , $org_charset = null ){
        ...
        if ($target_chrset === 'ISO-2022-JP' && PHP_VERSION_ID >= 50201) {
            $target_chrset = 'ISO-2022-JP-MS';
        }
...
#			$this->bcc( false ) ;
			if( empty( $tos ) ){
				$fg = $this->errorGather('recipient Header is not exist line' ,__LINE__) ;
			}else{
				// To Separate mode
...
$this->debugEchoLf($this->to);
	...
	function sendBase(){
		// stack bcc for always bcc
		unset( $stack_bcc ) ;
		if( !empty( $this->allways_bcc ) ){
			$stack_bcc = $this->bcc ;
...
//			$this->content_for_mailfunction,
	...
	function signSmime(){
		if( !$this->smtp ){
			return $this->errorGather('S/MIME needs SMTP Send,now You specify no smtp',__LINE__);
		}
		...
		if('PFX'===strtoupper(substr($this->certificate_file,-3))){
			if(!function_exists('openssl_pkcs12_read')){
				return $this->errorGather('You can not specify *.pfx type, please *.pem type because your PHP Version do not support \'openssl_pkcs12_read\'',__LINE__);
			}
	...
	function mimeEncode( $subject , $charset , $org_charset = null , $first_line_front_words_length = 12 ) {
        ...
        // , （半角カンマ）があれば MIME Encode する。
        $forceMimeEncode = (strpos($subject, ',') !== false);

        if( !$forceMimeEncode && (empty($subject) || ( strlen(bin2hex($subject))/2 == mb_strlen($subject,$enc) ) ) ){
        ...
        if ($charset === 'ISO-2022-JP' && PHP_VERSION_ID >= 50201) {
	...
	function extractAddr($addr_including_sclub){
		...
		if( $fg ){
			...
			return $this->errorGather('Illegal Mail Address'.$mess.'Validate Address Method' ,__LINE__) ;
		}
	}
	//----------------------------------------------------------------
...
EOF;
	...
	function sendBySendmail(){
		...
		$fg = unlink($temp_name);
		if(false===$fg){
			return $this->errorGather('Can not delete Temporary File ',__LINE__);
		}
	...
	function & smtpObject( $null = false ){
		...
		if( !class_exists ( 'Qdsmtp' ) && file_exists( 'qdsmtp.php' ) ){
			...
			return $this->errorGather('Please load SMTP Program - Qdsmtp http://hal456.net/qdsmtp',__LINE__);
		}
...
class QdmailUserFunc extends QdmailBase{
	...
	function validateAddr( $addr ){
		if(0==preg_match( $this->varidate_address_regex , $addr , $match )){
			$this->errorGather('Try Varidate Error by regex preg_match(\''.$this->varidate_address_regex . '\') the address is ->'.$addr,__LINE__);
		}else{
			return true;
		}
	}
